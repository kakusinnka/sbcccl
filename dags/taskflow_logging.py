...
def taskflow_logging():
    ...
    @task
    def task_start():
        logger.log_text('taskflow-logging' + 'DAG 开始')

    @task()
    def task_001():
        logger.log_text('taskflow-logging' + ' TASK001 开始')
        print('API调用开始')
        logger.log_text('taskflow-logging' + ' TASK001 结束')

    @task
    def task_end():
        logger.log_text('taskflow-logging' + ' DAG 结束')
